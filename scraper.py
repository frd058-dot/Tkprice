    ...
    try:
        with open(file_name, 'r', encoding='utf-8') as f:
            ...
            for item in items:
                ...
                if found_price:
                    try:
                        # Rakamı çekip kontrol et (Örn: '4.200 TL' -> 4200)
                        clean_digit = "".join(filter(str.isdigit, found_price.split(',')[0]))
                        numeric_price = int(clean_digit) if clean_digit else 0
                        
                        # Eğer fiyat mantıksız derecede düşükse (Örn: < 400 TL) kaydetme
                        if numeric_price > 400:
                            results[pid] = {"price_tr": found_price}
                            print(f"    => OK: {found_price} ({full_name[:20]})")
                        else:
                            print(f"    => Hata: Çok düşük fiyat ({found_price})")
                    except:
                        print(f"    => Format Hatası: {found_price}")
                else:
                    print(f"    => BULUNAMADI")
...
print(f"\nBİTTİ! Toplam {len(results)} ürün güncellendi.")import requests, json, os, time
...
print(f"\nBİTTİ! Toplam {len(results)} ürün güncellendi.")
